...
#--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*
def reading_data_residual(freq_str, unit, Nside):
    Nside = int(Nside)
    ...
    residual_map = rebeamed_inpainted_sky_map - public_function.CMB_map(freq_str, unit, Nside) - public_function.reading_dust("Planck15-G", freq_str, unit, Nside) - public_function.reading_synch("Planck", freq_str, unit, Nside) - public_function.reading_AME(freq_str, unit, Nside) - public_function.reading_ff(freq_str, unit, Nside) - public_function.reading_xline(freq_str, unit, Nside)
    healpy.write_map(root_dir+"/results_residual/residual_data_"+unit+"_"+freq_str+"GHz.fits", residual_map, nest = False, coord = "G", column_units = unit, overwrite="True", dtype=numpy.float64)
    return 1

#--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*
Nside = 512

if os.path.exists(root_dir+"/results_residual/"):
   shutil.rmtree(root_dir+"/results_residual/")
os.mkdir(root_dir+"/results_residual/")

print("residual map from observed data: ")
...
arguments = [("030", "muK_CMB", Nside), 
             ...
             ("857", "muK_CMB", Nside),
             ("857", "MJy_sr",  Nside)]
multiprocessing.Pool(processes=10).starmap(reading_data_residual, arguments)

for freq_str in ["030", "044", "070", "100", "143", "217", "353", "545", "857"]:
    if os.path.isfile(root_dir+"/results_residual/residual_data_muK_CMB_"+freq_str+"GHz.fits"):
        pass
    else:
        reading_data_residual(freq_str, "muK_CMB", Nside)
end_time = time.time()
